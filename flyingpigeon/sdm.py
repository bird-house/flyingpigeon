...
def get_gbif(taxon_name='Fagus sylvatica', bbox=[-10,-10,10,10]):
  ...
  from numpy import nan, empty, linspace
  ...
  try:
    ...
    for x in linspace(bbox[0], bbox[2], num=int(round(x_len+0.5))):
      for y in linspace(bbox[1], bbox[3], num=int(round(y_len+0.5))):
        #print 'processing %s , %s' % (x,y)
        poly = "POLYGON ((%s %s,%s %s,%s %s,%s %s,%s %s))" % (x,y, x,y+y_len, x + x_len, y+y_len, x+x_len, y, x,y)
        polys.extend([poly])
    # print(polys)  
