...
def call(resource=[], variable=None, dimension_map=None, calc=None,
         calc_grouping=None, conform_units_to=None, memory_limit=None,
         prefix=None, regrid_destination=None, regrid_options='bil',
         level_range=None, geom=None, output_format_options=False,
         search_radius_mult=2., select_nearest=False, select_ugid=None,
         spatial_wrapping=None, t_calendar=None, time_region=None,
    ...
    :param conform_units_to: transformation of units
    :param memory_limit: limit the amount of data to be loaded into the memory\
                at once if None (default) free memory is detected by birdhouse
    :param level_range: subset of given levels
    :param prefix: string for the file base name
    :param regrid_destination: file path with netCDF file with grid for output
    ...
    :param search_radius_mult: search radius for point geometries. All\
        included gridboxes will be returned
    ...
    env.PREFIX = prefix
    if output_format_options is not False:
        output_format_options = None
    elif output_format_options is not True:
        output_format_options = {
            'data_model': 'NETCDF4',  # NETCDF4_CLASSIC
    ...
    if has_Lambert_Conformal(resource) is True and not geom is None:
    ...
    output = None
        ...
        try:
            ...
            if variable is None:
                ...
                # data_kb = size['total']/reduce(lambda x,y: x*y,
                #               size['variables'][variable]['value']['shape'])
            logger.info('data_mb  = %s ; memory_limit = %s '
                        % (data_mb, mem_limit))
        except Exception as e:
            logger.debug('failed to compare dataload with free memory %s ' % e)
            raise

        # process 'en block' or in chunks
        if data_mb <= mem_limit:
            # input is smaller than the half of free memory size
            try:
