...
## Addition d'un cycle saisonnier a des anomalies
## Demande d'avoir calcule le cycle saisonnier avec soussseasmean
"addseasmean" <- function(dat,conv.time,seascyc)
  {
    dat.m=dat
    time.cyc=seascyc$timecyc
    seas.cyc.spl=seascyc$seascyc
    for(t in 1:nrow(dat)){
      mon=conv.time$month[t]
      day=conv.time$day[t]
      ii= which(time.cyc$month %in% mon & time.cyc$day %in% day)
...
##------------------------------------------------------------------------
## Soustraction d'un champ saisonnier moyen aux donnees
"sousfield"<-function(dat,conv.time,l.mon=1:12,
                         seas.cyc.spl=NULL,time.cyc,rprint=FALSE)
{
  dat.m=dat
  if(length(seas.cyc.spl)==0){
      seas.cyc=c()
      time.cyc=c()
      ## Calcul du cycle saisonnier pour une liste d'annees dans l.year
      if(rprint) print("Seasonal cycle")
...
## Calcul d'un cycle saisonner pour une series temporelles
"seascyc"<-function(X,time,l.year=c(1970:2000),fun.type="mean")
  {
    FUN=match.fun(fun.type)
    I=which(time>=(l.year[1]*10000+100+1) & time<=(l.year[2]*10000+1200+31))
    mmdd=time %% 10000
    Xseas=tapply(X[I],mmdd[I],FUN,na.rm=TRUE)
    Xdum=c(Xseas,Xseas,Xseas)
    Xsmo=smooth.spline(Xdum,spar=0.8)
    l=length(Xsmo$y)/3
    return(Xsmo$y[(l+1):(2*l)])
  }
