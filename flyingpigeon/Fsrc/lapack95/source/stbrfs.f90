...
PURE SUBROUTINE STBRFS_F95(AB,B,X,UPLO,TRANS,DIAG,FERR,BERR,INFO)
    ...
    ! DIAG='N','U'; default: 'N'
    ! <<< Use statements >>>
    USE F77_LAPACK, ONLY: F77_TBRFS, F77_XERBLA
    ! <<< ENTRY point >>>
    ENTRY STBRFS_MKL95(AB,B,X,UPLO,TRANS,DIAG,FERR,BERR,INFO)
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
    ...
    ! <<< Local declarations >>>
    ! <<< Parameters >>>
    CHARACTER(LEN=5), PARAMETER :: SRNAME = 'TBRFS'
    ! <<< Local scalars >>>
    ...
    IF(L_STAT_ALLOC==0) THEN
        CALL F77_TBRFS(O_UPLO,O_TRANS,O_DIAG,N,KD,NRHS,AB,LDAB,B,LDB,X, &
     &                              LDX,O_FERR,O_BERR,WORK,IWORK,O_INFO)
    ELSE; O_INFO = -1000
    ENDIF
