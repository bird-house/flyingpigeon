...
PURE SUBROUTINE DSPTRD_F95(AP,TAU,UPLO,INFO)
    ...
    ! UPLO='U','L'; default: 'U'
    ! <<< Use statements >>>
    USE F77_LAPACK, ONLY: F77_SPTRD, F77_XERBLA
    ! <<< ENTRY point >>>
    ENTRY DSPTRD_MKL95(AP,TAU,UPLO,INFO)
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
    ...
    ! <<< Local declarations >>>
    ! <<< Parameters >>>
    CHARACTER(LEN=5), PARAMETER :: SRNAME = 'SPTRD'
    ! <<< Local scalars >>>
    ...
    IF(L_STAT_ALLOC==0) THEN
        CALL F77_SPTRD(O_UPLO,N,AP,D,E,TAU,O_INFO)
    ELSE; O_INFO = -1000
    ENDIF
