...
PURE SUBROUTINE ZGGESX_F95(A,B,ALPHA,BETA,VSL,VSR,SELECT,SDIM,RCONDE,   &
     &                                                      RCONDV,INFO)
    ! Fortran77 call:
    ! ZGGESX(JOBVSL,JOBVSR,SORT,SELECT,SENSE,N,A,LDA,B,LDB,SDIM,ALPHA,
    !   BETA,VSL,LDVSL,VSR,LDVSR,RCONDE,RCONDV,WORK,LWORK,RWORK,IWORK,
    !   LIWORK,BWORK,INFO)
    ! <<< Use statements >>>
    USE F77_LAPACK, ONLY: F77_GGESX, F77_XERBLA
    ! <<< ENTRY point >>>
    ENTRY ZGGESX_MKL95(A,B,ALPHA,BETA,VSL,VSR,SELECT,SDIM,RCONDE,RCONDV,&
     &                                                             INFO)
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
