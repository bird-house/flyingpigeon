...
PURE SUBROUTINE SSPEVD_F95(AP,W,UPLO,Z,INFO)
    ...
    ! UPLO='U','L'; default: 'U'
    ! <<< Use statements >>>
    USE F77_LAPACK, ONLY: F77_SPEVD, F77_XERBLA
    ! <<< ENTRY point >>>
    ENTRY SSPEVD_MKL95(AP,W,UPLO,Z,INFO)
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
    ...
    ! <<< Local declarations >>>
    ! <<< Parameters >>>
    CHARACTER(LEN=5), PARAMETER :: SRNAME = 'SPEVD'
    ! <<< Local scalars >>>
    ...
    LIWORK = -1
    LWORK = -1
    CALL F77_SPEVD(JOBZ,O_UPLO,N,AP,W,O_Z,LDZ,S_WORK,LWORK,S_IWORK,     &
     &                                                    LIWORK,O_INFO)
    ...
    IF(L_STAT_ALLOC==0) THEN
        CALL F77_SPEVD(JOBZ,O_UPLO,N,AP,W,O_Z,LDZ,WORK,LWORK,IWORK,     &
     &                                                    LIWORK,O_INFO)
    ELSE; O_INFO = -1000
    ENDIF
