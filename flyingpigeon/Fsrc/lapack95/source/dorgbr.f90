...
PURE SUBROUTINE DORGBR_F95(A,TAU,VECT,INFO)
    ...
    ! VECT='Q','P'; default: 'Q'
    ! <<< Use statements >>>
    USE F77_LAPACK, ONLY: F77_ORGBR, F77_XERBLA
    ! <<< ENTRY point >>>
    ENTRY DORGBR_MKL95(A,TAU,VECT,INFO)
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
    ...
    ! <<< Local declarations >>>
    ! <<< Parameters >>>
    CHARACTER(LEN=5), PARAMETER :: SRNAME = 'ORGBR'
    ! <<< Local scalars >>>
    ...
    LWORK = -1
    CALL F77_ORGBR(O_VECT,M,N,K,A,LDA,TAU,S_WORK,LWORK,O_INFO)
    ...
    IF(L_STAT_ALLOC==0) THEN
        CALL F77_ORGBR(O_VECT,M,N,K,A,LDA,TAU,WORK,LWORK,O_INFO)
    ELSE; O_INFO = -1000
    ENDIF
