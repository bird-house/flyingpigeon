...
PURE SUBROUTINE ZHETRD_F95(A,TAU,UPLO,INFO)
    ...
    ! UPLO='U','L'; default: 'U'
    ! <<< Use statements >>>
    USE F77_LAPACK, ONLY: F77_HETRD, F77_XERBLA
    ! <<< ENTRY point >>>
    ENTRY ZHETRD_MKL95(A,TAU,UPLO,INFO)
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
    ...
    ! <<< Local declarations >>>
    ! <<< Parameters >>>
    CHARACTER(LEN=5), PARAMETER :: SRNAME = 'HETRD'
    ! <<< Local scalars >>>
    ...
    LWORK = -1
    CALL F77_HETRD(O_UPLO,N,A,LDA,D,E,TAU,S_WORK,LWORK,O_INFO)
    ...
    IF(L_STAT_ALLOC==0) THEN
        CALL F77_HETRD(O_UPLO,N,A,LDA,D,E,TAU,WORK,LWORK,O_INFO)
    ELSE; O_INFO = -1000
    ENDIF
