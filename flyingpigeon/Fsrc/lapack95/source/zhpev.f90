...
PURE SUBROUTINE ZHPEV_F95(AP,W,UPLO,Z,INFO)
    ...
    ! UPLO='U','L'; default: 'U'
    ! <<< Use statements >>>
    USE F77_LAPACK, ONLY: F77_HPEV, F77_XERBLA
    ! <<< ENTRY point >>>
    ENTRY ZHPEV_MKL95(AP,W,UPLO,Z,INFO)
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
    ...
    ! <<< Local declarations >>>
    ! <<< Parameters >>>
    CHARACTER(LEN=4), PARAMETER :: SRNAME = 'HPEV'
    ! <<< Local scalars >>>
    ...
    IF(L_STAT_ALLOC==0) THEN
        CALL F77_HPEV(JOBZ,O_UPLO,N,AP,W,O_Z,LDZ,WORK,RWORK,O_INFO)
    ELSE; O_INFO = -1000
    ENDIF
