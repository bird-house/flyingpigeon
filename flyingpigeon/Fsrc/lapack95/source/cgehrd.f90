...
PURE SUBROUTINE CGEHRD_F95(A,TAU,ILO,IHI,INFO)
    ...
    ! Default IHI=N
    ! <<< Use statements >>>
    USE F77_LAPACK, ONLY: F77_GEHRD, F77_XERBLA
    ! <<< ENTRY point >>>
    ENTRY CGEHRD_MKL95(A,TAU,ILO,IHI,INFO)
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
    ...
    ! <<< Local declarations >>>
    ! <<< Parameters >>>
    CHARACTER(LEN=5), PARAMETER :: SRNAME = 'GEHRD'
    ! <<< Local scalars >>>
    INTEGER :: O_ILO
    INTEGER :: O_IHI
    INTEGER :: O_INFO
    ...
    IF(PRESENT(TAU)) THEN
        O_TAU => TAU
    ELSE
        ALLOCATE(O_TAU(N-1), STAT=L_STAT_ALLOC)
    ENDIF
    ! <<< Request work array(s) size >>>
    LWORK = -1
    CALL F77_GEHRD(N,O_ILO,O_IHI,A,LDA,O_TAU,S_WORK,LWORK,O_INFO)
    ...
    IF(L_STAT_ALLOC==0) THEN
        CALL F77_GEHRD(N,O_ILO,O_IHI,A,LDA,O_TAU,WORK,LWORK,O_INFO)
    ELSE; O_INFO = -1000
    ENDIF
