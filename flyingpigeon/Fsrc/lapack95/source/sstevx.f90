...
PURE SUBROUTINE SSTEVX_F95(D,E,W,Z,VL,VU,IL,IU,M,IFAIL,ABSTOL,INFO)
    ! Fortran77 call:
    ! SSTEVX(JOBZ,RANGE,N,D,E,VL,VU,IL,IU,ABSTOL,M,W,Z,LDZ,WORK,IWORK,
    !   IFAIL,INFO)
    ! Default VL=-HUGE(VL)
    ! Default VU=HUGE(VL)
    ! Default IL=1
    ...
    ! <<< Use statements >>>
    USE F77_LAPACK, ONLY: F77_STEVX, F77_XERBLA
    ! <<< ENTRY point >>>
    ENTRY SSTEVX_MKL95(D,E,W,Z,VL,VU,IL,IU,M,IFAIL,ABSTOL,INFO)
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
    ...
    ! <<< Local declarations >>>
    ! <<< Parameters >>>
    CHARACTER(LEN=5), PARAMETER :: SRNAME = 'STEVX'
    ! <<< Local scalars >>>
    REAL(WP) :: O_VL
    REAL(WP) :: O_VU
    INTEGER :: O_IL
    ...
    IF(L_STAT_ALLOC==0) THEN
        CALL F77_STEVX(JOBZ,RANGE,N,D,E,O_VL,O_VU,O_IL,O_IU,O_ABSTOL,   &
     &                          O_M,W,O_Z,LDZ,WORK,IWORK,O_IFAIL,O_INFO)
    ELSE; O_INFO = -1000
    ENDIF
