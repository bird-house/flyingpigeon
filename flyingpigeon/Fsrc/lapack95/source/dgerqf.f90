...
PURE SUBROUTINE DGERQF_F95(A,TAU,INFO)
    ...
    ! <<< Use statements >>>
    USE F77_LAPACK, ONLY: F77_GERQF, F77_XERBLA
    ! <<< ENTRY point >>>
    ENTRY DGERQF_MKL95(A,TAU,INFO)
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
    ...
    ! <<< Local declarations >>>
    ! <<< Parameters >>>
    CHARACTER(LEN=5), PARAMETER :: SRNAME = 'GERQF'
    ! <<< Local scalars >>>
    INTEGER :: O_INFO
    INTEGER :: M
    INTEGER :: N
    ...
    LWORK = -1
    CALL F77_GERQF(M,N,A,LDA,O_TAU,S_WORK,LWORK,O_INFO)
    ...
    IF(L_STAT_ALLOC==0) THEN
        CALL F77_GERQF(M,N,A,LDA,O_TAU,WORK,LWORK,O_INFO)
    ELSE; O_INFO = -1000
    ENDIF
