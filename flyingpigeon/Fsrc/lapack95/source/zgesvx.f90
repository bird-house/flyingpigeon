...
PURE SUBROUTINE ZGESVX_F95(A,B,X,AF,IPIV,FACT,TRANS,EQUED,R,C,FERR,BERR,&
     &                                                RCOND,RPVGRW,INFO)
    ! Fortran77 call:
    ! ZGESVX(FACT,TRANS,N,NRHS,A,LDA,AF,LDAF,IPIV,EQUED,R,C,B,LDB,X,LDX,
    ...
    ! <<< ENTRY point >>>
    ENTRY ZGESVX_MKL95(A,B,X,AF,IPIV,FACT,TRANS,EQUED,R,C,FERR,BERR,    &
     &                                                RCOND,RPVGRW,INFO)
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
    ...
    IF(L_STAT_ALLOC==0) THEN
     ...
     &    O_EQUED,O_R,O_C,B,LDB,X,LDX,O_RCOND,O_FERR,O_BERR,WORK,RWORK, &
     &                                                           O_INFO)
    ELSE; O_INFO = -1000
    ENDIF
