...
PURE SUBROUTINE CGEQP3_F95(A,JPVT,TAU,INFO)
    ...
    ! <<< Use statements >>>
    USE F77_LAPACK, ONLY: F77_GEQP3, F77_XERBLA
    ! <<< ENTRY point >>>
    ENTRY CGEQP3_MKL95(A,JPVT,TAU,INFO)
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
    ...
    ! <<< Local declarations >>>
    ! <<< Parameters >>>
    CHARACTER(LEN=5), PARAMETER :: SRNAME = 'GEQP3'
    ! <<< Local scalars >>>
    INTEGER :: O_INFO
    INTEGER :: M
    INTEGER :: N
    ...
    LWORK = -1
    CALL F77_GEQP3(M,N,A,LDA,JPVT,O_TAU,S_WORK,LWORK,RWORK,O_INFO)
    ...
    IF(L_STAT_ALLOC==0) THEN
        CALL F77_GEQP3(M,N,A,LDA,JPVT,O_TAU,WORK,LWORK,RWORK,O_INFO)
    ELSE; O_INFO = -1000
    ENDIF
