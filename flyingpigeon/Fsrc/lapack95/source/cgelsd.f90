...
PURE SUBROUTINE CGELSD_F95(A,B,RANK,S,RCOND,INFO)
    ! Fortran77 call:
    ! CGELSD(M,N,NRHS,A,LDA,B,LDB,S,RCOND,RANK,WORK,LWORK,RWORK,IWORK,
    !   INFO)
    ! Default RCOND=100*EPSILON(1.0_WP)
    ! <<< Use statements >>>
    USE F77_LAPACK, ONLY: F77_GELSD, F77_XERBLA, F77_ILAENV
    ! <<< ENTRY point >>>
    ENTRY CGELSD_MKL95(A,B,RANK,S,RCOND,INFO)
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
    ...
    N = SIZE(A,2)
    NRHS = SIZE(B,2)
    SMLSIZ = F77_ILAENV(9,'CGELSD',' ', 0,0,0,0)
    MINMN = MAX(1,MIN(M,N))
    NLVL = MAX(INT(LOG(DBLE(MINMN)/DBLE(SMLSIZ+1))/                     &
     &                                           LOG(DBLE(2.0_WP)))+1,0)
    LIWORK = 3*MINMN*(NLVL+1)+11*MINMN
    LRWORK = 10*MINMN+2*MINMN*SMLSIZ+8*MINMN*NLVL+3*SMLSIZ*             &
     &                                        NRHS+(SMLSIZ+1)*(SMLSIZ+1)
