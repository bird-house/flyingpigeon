...
PURE SUBROUTINE CHERFS_F95(A,AF,IPIV,B,X,UPLO,FERR,BERR,INFO)
    ! Fortran77 call:
    ! CHERFS(UPLO,N,NRHS,A,LDA,AF,LDAF,IPIV,B,LDB,X,LDX,FERR,BERR,WORK,
    !   RWORK,INFO)
    ! UPLO='U','L'; default: 'U'
    ! <<< Use statements >>>
    USE F77_LAPACK, ONLY: F77_HERFS, F77_XERBLA
    ! <<< ENTRY point >>>
    ENTRY CHERFS_MKL95(A,AF,IPIV,B,X,UPLO,FERR,BERR,INFO)
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
    ...
    ! <<< Local declarations >>>
    ! <<< Parameters >>>
    CHARACTER(LEN=5), PARAMETER :: SRNAME = 'HERFS'
    ! <<< Local scalars >>>
    ...
    IF(L_STAT_ALLOC==0) THEN
        CALL F77_HERFS(O_UPLO,N,NRHS,A,LDA,AF,LDAF,IPIV,B,LDB,X,LDX,    &
     &                                  O_FERR,O_BERR,WORK,RWORK,O_INFO)
    ELSE; O_INFO = -1000
    ENDIF
