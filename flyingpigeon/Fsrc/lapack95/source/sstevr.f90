...
PURE SUBROUTINE SSTEVR_F95(D,E,W,Z,VL,VU,IL,IU,M,ISUPPZ,ABSTOL,INFO)
    ! Fortran77 call:
    ! SSTEVR(JOBZ,RANGE,N,D,E,VL,VU,IL,IU,ABSTOL,M,W,Z,LDZ,ISUPPZ,WORK,
    ...
    ! <<< Use statements >>>
    USE F77_LAPACK, ONLY: F77_STEVR, F77_XERBLA
    ! <<< ENTRY point >>>
    ENTRY SSTEVR_MKL95(D,E,W,Z,VL,VU,IL,IU,M,ISUPPZ,ABSTOL,INFO)
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
    ...
    ! <<< Local declarations >>>
    ! <<< Parameters >>>
    CHARACTER(LEN=5), PARAMETER :: SRNAME = 'STEVR'
    ! <<< Local scalars >>>
    REAL(WP) :: O_VL
    REAL(WP) :: O_VU
    INTEGER :: O_IL
    ...
    LIWORK = -1
    LWORK = -1
    CALL F77_STEVR(JOBZ,RANGE,N,D,E,O_VL,O_VU,O_IL,O_IU,O_ABSTOL,O_M,W, &
    ...
    IF(L_STAT_ALLOC==0) THEN
        CALL F77_STEVR(JOBZ,RANGE,N,D,E,O_VL,O_VU,O_IL,O_IU,O_ABSTOL,   &
