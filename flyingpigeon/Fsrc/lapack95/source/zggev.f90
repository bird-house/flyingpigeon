...
PURE SUBROUTINE ZGGEV_F95(A,B,ALPHA,BETA,VL,VR,INFO)
    ! Fortran77 call:
    ! ZGGEV(JOBVL,JOBVR,N,A,LDA,B,LDB,ALPHA,BETA,VL,LDVL,VR,LDVR,WORK,
    !   LWORK,RWORK,INFO)
    ! <<< Use statements >>>
    USE F77_LAPACK, ONLY: F77_GGEV, F77_XERBLA
    ! <<< ENTRY point >>>
    ENTRY ZGGEV_MKL95(A,B,ALPHA,BETA,VL,VR,INFO)
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
    ...
    LWORK = -1
    CALL F77_GGEV(JOBVL,JOBVR,N,A,LDA,B,LDB,ALPHA,BETA,O_VL,LDVL,O_VR,  &
     &                                   LDVR,S_WORK,LWORK,RWORK,O_INFO)
    ...
    IF(L_STAT_ALLOC==0) THEN
        CALL F77_GGEV(JOBVL,JOBVR,N,A,LDA,B,LDB,ALPHA,BETA,O_VL,LDVL,   &
     &                                O_VR,LDVR,WORK,LWORK,RWORK,O_INFO)
    ELSE; O_INFO = -1000
    ENDIF
