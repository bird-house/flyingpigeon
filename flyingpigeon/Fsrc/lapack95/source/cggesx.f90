...
PURE SUBROUTINE CGGESX_F95(A,B,ALPHA,BETA,VSL,VSR,SELECT,SDIM,RCONDE,   &
     &                                                      RCONDV,INFO)
    ! Fortran77 call:
    ! CGGESX(JOBVSL,JOBVSR,SORT,SELECT,SENSE,N,A,LDA,B,LDB,SDIM,ALPHA,
    !   BETA,VSL,LDVSL,VSR,LDVSR,RCONDE,RCONDV,WORK,LWORK,RWORK,IWORK,
    !   LIWORK,BWORK,INFO)
    ! <<< Use statements >>>
    USE F77_LAPACK, ONLY: F77_GGESX, F77_XERBLA
    ! <<< ENTRY point >>>
    ENTRY CGGESX_MKL95(A,B,ALPHA,BETA,VSL,VSR,SELECT,SDIM,RCONDE,RCONDV,&
     &                                                             INFO)
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
    ...
    IF(L_STAT_ALLOC==0) THEN
     ...
     &O_SDIM,ALPHA,BETA,O_VSL,LDVSL,O_VSR,LDVSR,O_RCONDE,O_RCONDV,WORK, &
     &                            LWORK,RWORK,IWORK,LIWORK,BWORK,O_INFO)
    ELSE; O_INFO = -1000
    ENDIF
