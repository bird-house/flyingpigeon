...
PURE SUBROUTINE SSPRFS_F95(AP,AFP,IPIV,B,X,UPLO,FERR,BERR,INFO)
    ! Fortran77 call:
    ! SSPRFS(UPLO,N,NRHS,AP,AFP,IPIV,B,LDB,X,LDX,FERR,BERR,WORK,IWORK,
    !   INFO)
    ! UPLO='U','L'; default: 'U'
    ! <<< Use statements >>>
    USE F77_LAPACK, ONLY: F77_SPRFS, F77_XERBLA
    ! <<< ENTRY point >>>
    ENTRY SSPRFS_MKL95(AP,AFP,IPIV,B,X,UPLO,FERR,BERR,INFO)
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
    ...
    IF(L_STAT_ALLOC==0) THEN
        CALL F77_SPRFS(O_UPLO,N,NRHS,AP,AFP,IPIV,B,LDB,X,LDX,O_FERR,    &
