...
PURE SUBROUTINE ZHPTRF_F95(AP,UPLO,IPIV,INFO)
    ...
    ! UPLO='U','L'; default: 'U'
    ! <<< Use statements >>>
    USE F77_LAPACK, ONLY: F77_HPTRF, F77_XERBLA
    ! <<< ENTRY point >>>
    ENTRY ZHPTRF_MKL95(AP,UPLO,IPIV,INFO)
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
    ...
    ! <<< Local declarations >>>
    ! <<< Parameters >>>
    CHARACTER(LEN=5), PARAMETER :: SRNAME = 'HPTRF'
    ! <<< Local scalars >>>
    ...
    IF(L_STAT_ALLOC==0) THEN
        CALL F77_HPTRF(O_UPLO,N,AP,O_IPIV,O_INFO)
    ELSE; O_INFO = -1000
    ENDIF
