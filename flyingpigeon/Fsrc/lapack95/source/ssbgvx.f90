...
PURE SUBROUTINE SSBGVX_F95(AB,BB,W,UPLO,Z,VL,VU,IL,IU,M,IFAIL,Q,ABSTOL, &
     &                                                             INFO)
    ! Fortran77 call:
    ! SSBGVX(JOBZ,RANGE,UPLO,N,KA,KB,AB,LDAB,BB,LDBB,Q,LDQ,VL,VU,IL,IU,
    ...
    ! <<< Use statements >>>
    USE F77_LAPACK, ONLY: F77_SBGVX, F77_XERBLA
    ! <<< ENTRY point >>>
    ENTRY SSBGVX_MKL95(AB,BB,W,UPLO,Z,VL,VU,IL,IU,M,IFAIL,Q,ABSTOL,INFO)
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
    ...
    ! <<< Local declarations >>>
    ! <<< Parameters >>>
    CHARACTER(LEN=5), PARAMETER :: SRNAME = 'SBGVX'
    ! <<< Local scalars >>>
    ...
    IF(L_STAT_ALLOC==0) THEN
        CALL F77_SBGVX(JOBZ,RANGE,O_UPLO,N,KA,KB,AB,LDAB,BB,LDBB,O_Q,   &
     &LDQ,O_VL,O_VU,O_IL,O_IU,O_ABSTOL,O_M,W,O_Z,LDZ,WORK,IWORK,O_IFAIL,&
     &                                                           O_INFO)
    ELSE; O_INFO = -1000
    ENDIF
