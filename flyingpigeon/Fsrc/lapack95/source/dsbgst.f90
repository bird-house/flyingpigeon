...
PURE SUBROUTINE DSBGST_F95(AB,BB,X,UPLO,INFO)
    ...
    ! UPLO='U','L'; default: 'U'
    ! <<< Use statements >>>
    USE F77_LAPACK, ONLY: F77_SBGST, F77_XERBLA
    ! <<< ENTRY point >>>
    ENTRY DSBGST_MKL95(AB,BB,X,UPLO,INFO)
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
    ...
    ! <<< Local declarations >>>
    ! <<< Parameters >>>
    CHARACTER(LEN=5), PARAMETER :: SRNAME = 'SBGST'
    ! <<< Local scalars >>>
    ...
    IF(L_STAT_ALLOC==0) THEN
        CALL F77_SBGST(VECT,O_UPLO,N,KA,KB,AB,LDAB,BB,LDBB,O_X,LDX,WORK,&
     &                                                           O_INFO)
    ELSE; O_INFO = -1000
    ENDIF
