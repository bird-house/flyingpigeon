...
PURE SUBROUTINE DSYEVR_F95(A,W,UPLO,Z,VL,VU,IL,IU,M,ISUPPZ,ABSTOL,INFO)
    ! Fortran77 call:
    ...
    ! Default ABSTOL=0.0_WP
    ! <<< Use statements >>>
    USE F77_LAPACK, ONLY: F77_SYEVR, F77_XERBLA
    ! <<< ENTRY point >>>
    ENTRY DSYEVR_MKL95(A,W,UPLO,Z,VL,VU,IL,IU,M,ISUPPZ,ABSTOL,INFO)
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
    ...
    LIWORK = -1
    LWORK = -1
    CALL F77_SYEVR(JOBZ,RANGE,O_UPLO,N,A,LDA,O_VL,O_VU,O_IL,O_IU,       &
     &     O_ABSTOL,O_M,W,O_Z,LDZ,O_ISUPPZ,S_WORK,LWORK,S_IWORK,LIWORK, &
     &                                                           O_INFO)
    ...
    IF(L_STAT_ALLOC==0) THEN
        CALL F77_SYEVR(JOBZ,RANGE,O_UPLO,N,A,LDA,O_VL,O_VU,O_IL,O_IU,   &
     &   O_ABSTOL,O_M,W,O_Z,LDZ,O_ISUPPZ,WORK,LWORK,IWORK,LIWORK,O_INFO)
    ELSE; O_INFO = -1000
    ENDIF
