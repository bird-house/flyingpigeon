...
PURE SUBROUTINE CSTEDC_F95(D,E,Z,COMPZ,INFO)
    ...
    ! <<< ENTRY point >>>
    ENTRY CSTEDC_MKL95(D,E,Z,COMPZ,INFO)
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
    ...
    LRWORK = -1
    LWORK = -1
    CALL F77_STEDC(O_COMPZ,N,D,E,O_Z,LDZ,S_WORK,LWORK,S_RWORK,LRWORK,   &
    ...
    IF(L_STAT_ALLOC==0) THEN
        CALL F77_STEDC(O_COMPZ,N,D,E,O_Z,LDZ,WORK,LWORK,RWORK,LRWORK,   &
     &                                              IWORK,LIWORK,O_INFO)
    ELSE; O_INFO = -1000
    ENDIF
