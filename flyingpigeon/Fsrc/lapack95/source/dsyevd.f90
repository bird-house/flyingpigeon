...
PURE SUBROUTINE DSYEVD_F95(A,W,JOBZ,UPLO,INFO)
    ...
    ! UPLO='U','L'; default: 'U'
    ! <<< Use statements >>>
    USE F77_LAPACK, ONLY: F77_SYEVD, F77_XERBLA
    ! <<< ENTRY point >>>
    ENTRY DSYEVD_MKL95(A,W,JOBZ,UPLO,INFO)
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
    ...
    LIWORK = -1
    LWORK = -1
    CALL F77_SYEVD(O_JOBZ,O_UPLO,N,A,LDA,W,S_WORK,LWORK,S_IWORK,LIWORK, &
     &                                                           O_INFO)
    ...
    IF(L_STAT_ALLOC==0) THEN
        CALL F77_SYEVD(O_JOBZ,O_UPLO,N,A,LDA,W,WORK,LWORK,IWORK,LIWORK, &
     &                                                           O_INFO)
    ELSE; O_INFO = -1000
    ENDIF
