...
PURE SUBROUTINE CPPCON_F95(AP,ANORM,RCOND,UPLO,INFO)
    ...
    ! UPLO='U','L'; default: 'U'
    ! <<< Use statements >>>
    USE F77_LAPACK, ONLY: F77_PPCON, F77_XERBLA
    ! <<< ENTRY point >>>
    ENTRY CPPCON_MKL95(AP,ANORM,RCOND,UPLO,INFO)
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
    ...
    ! <<< Local declarations >>>
    ! <<< Parameters >>>
    CHARACTER(LEN=5), PARAMETER :: SRNAME = 'PPCON'
    ! <<< Local scalars >>>
    ...
    IF(L_STAT_ALLOC==0) THEN
        CALL F77_PPCON(O_UPLO,N,AP,ANORM,RCOND,WORK,RWORK,O_INFO)
    ELSE; O_INFO = -1000
    ENDIF
