...
PURE SUBROUTINE CSPTRF_F95(AP,UPLO,IPIV,INFO)
    ...
    ! UPLO='U','L'; default: 'U'
    ! <<< Use statements >>>
    USE F77_LAPACK, ONLY: F77_SPTRF, F77_XERBLA
    ! <<< ENTRY point >>>
    ENTRY CSPTRF_MKL95(AP,UPLO,IPIV,INFO)
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
    ...
    ! <<< Local declarations >>>
    ! <<< Parameters >>>
    CHARACTER(LEN=5), PARAMETER :: SRNAME = 'SPTRF'
    ! <<< Local scalars >>>
    ...
    IF(L_STAT_ALLOC==0) THEN
        CALL F77_SPTRF(O_UPLO,N,AP,O_IPIV,O_INFO)
    ELSE; O_INFO = -1000
    ENDIF
