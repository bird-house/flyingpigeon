...
PURE SUBROUTINE ZHPGVX_F95(AP,BP,W,ITYPE,UPLO,Z,VL,VU,IL,IU,M,IFAIL,    &
     &                                                      ABSTOL,INFO)
    ! Fortran77 call:
    ! ZHPGVX(ITYPE,JOBZ,RANGE,UPLO,N,AP,BP,VL,VU,IL,IU,ABSTOL,M,W,Z,LDZ,
    ...
    ! <<< Use statements >>>
    USE F77_LAPACK, ONLY: F77_HPGVX, F77_XERBLA
    ! <<< ENTRY point >>>
    ENTRY ZHPGVX_MKL95(AP,BP,W,ITYPE,UPLO,Z,VL,VU,IL,IU,M,IFAIL,ABSTOL, &
     &                                                             INFO)
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
    ...
    ! <<< Local declarations >>>
    ! <<< Parameters >>>
    CHARACTER(LEN=5), PARAMETER :: SRNAME = 'HPGVX'
    ! <<< Local scalars >>>
    INTEGER :: O_ITYPE
    ...
    IF(L_STAT_ALLOC==0) THEN
        CALL F77_HPGVX(O_ITYPE,JOBZ,RANGE,O_UPLO,N,AP,BP,O_VL,O_VU,O_IL,&
     &      O_IU,O_ABSTOL,O_M,W,O_Z,LDZ,WORK,RWORK,IWORK,O_IFAIL,O_INFO)
    ELSE; O_INFO = -1000
    ENDIF
