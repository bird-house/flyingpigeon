...
PURE SUBROUTINE SORMRQ_F95(A,TAU,C,SIDE,TRANS,INFO)
    ...
    ! TRANS='N','T'; default: 'N'
    ! <<< Use statements >>>
    USE F77_LAPACK, ONLY: F77_ORMRQ, F77_XERBLA
    ! <<< ENTRY point >>>
    ENTRY SORMRQ_MKL95(A,TAU,C,SIDE,TRANS,INFO)
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
    ...
    LWORK = -1
    CALL F77_ORMRQ(O_SIDE,O_TRANS,M,N,K,A,LDA,TAU,C,LDC,S_WORK,LWORK,   &
     &                                                           O_INFO)
    ...
    IF(L_STAT_ALLOC==0) THEN
        CALL F77_ORMRQ(O_SIDE,O_TRANS,M,N,K,A,LDA,TAU,C,LDC,WORK,LWORK, &
     &                                                           O_INFO)
    ELSE; O_INFO = -1000
    ENDIF
