...
PURE SUBROUTINE CHESVX_F95(A,B,X,UPLO,AF,IPIV,FACT,FERR,BERR,RCOND,INFO)
    ! Fortran77 call:
    ! CHESVX(FACT,UPLO,N,NRHS,A,LDA,AF,LDAF,IPIV,B,LDB,X,LDX,RCOND,FERR,
    !   BERR,WORK,LWORK,RWORK,INFO)
    ! UPLO='U','L'; default: 'U'
    ! FACT='N','F'; default: 'N'
    ! <<< Use statements >>>
    USE F77_LAPACK, ONLY: F77_HESVX, F77_XERBLA
    ! <<< ENTRY point >>>
    ENTRY CHESVX_MKL95(A,B,X,UPLO,AF,IPIV,FACT,FERR,BERR,RCOND,INFO)
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
    ...
    ! <<< Local declarations >>>
    ! <<< Parameters >>>
    CHARACTER(LEN=5), PARAMETER :: SRNAME = 'HESVX'
    ! <<< Local scalars >>>
    ...
    LWORK = -1
    CALL F77_HESVX(O_FACT,O_UPLO,N,NRHS,A,LDA,O_AF,LDAF,O_IPIV,B,LDB,X, &
    ...
    IF(L_STAT_ALLOC==0) THEN
        CALL F77_HESVX(O_FACT,O_UPLO,N,NRHS,A,LDA,O_AF,LDAF,O_IPIV,B,   &
