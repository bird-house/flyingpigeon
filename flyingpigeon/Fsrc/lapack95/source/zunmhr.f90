...
PURE SUBROUTINE ZUNMHR_F95(A,TAU,C,ILO,IHI,SIDE,TRANS,INFO)
    ...
    ! TRANS='N','C'; default: 'N'
    ! <<< Use statements >>>
    USE F77_LAPACK, ONLY: F77_UNMHR, F77_XERBLA
    ! <<< ENTRY point >>>
    ENTRY ZUNMHR_MKL95(A,TAU,C,ILO,IHI,SIDE,TRANS,INFO)
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
    ...
    ! <<< Local declarations >>>
    ! <<< Parameters >>>
    CHARACTER(LEN=5), PARAMETER :: SRNAME = 'UNMHR'
    ! <<< Local scalars >>>
    INTEGER :: O_ILO
    INTEGER :: O_IHI
    ...
    LWORK = -1
    CALL F77_UNMHR(O_SIDE,O_TRANS,M,N,O_ILO,O_IHI,A,LDA,TAU,C,LDC,      &
     &                                              S_WORK,LWORK,O_INFO)
