...
PURE SUBROUTINE CHBEVX_F95(AB,W,UPLO,Z,VL,VU,IL,IU,M,IFAIL,Q,ABSTOL,    &
     &                                                             INFO)
    ! Fortran77 call:
    ! CHBEVX(JOBZ,RANGE,UPLO,N,KD,AB,LDAB,Q,LDQ,VL,VU,IL,IU,ABSTOL,M,W,
    ...
    ! <<< Use statements >>>
    USE F77_LAPACK, ONLY: F77_HBEVX, F77_XERBLA
    ! <<< ENTRY point >>>
    ENTRY CHBEVX_MKL95(AB,W,UPLO,Z,VL,VU,IL,IU,M,IFAIL,Q,ABSTOL,INFO)
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
    ...
    ! <<< Local declarations >>>
    ! <<< Parameters >>>
    CHARACTER(LEN=5), PARAMETER :: SRNAME = 'HBEVX'
    ! <<< Local scalars >>>
    ...
    IF(L_STAT_ALLOC==0) THEN
        CALL F77_HBEVX(JOBZ,RANGE,O_UPLO,N,KD,AB,LDAB,O_Q,LDQ,O_VL,O_VU,&
     & O_IL,O_IU,O_ABSTOL,O_M,W,O_Z,LDZ,WORK,RWORK,IWORK,O_IFAIL,O_INFO)
    ELSE; O_INFO = -1000
    ENDIF
