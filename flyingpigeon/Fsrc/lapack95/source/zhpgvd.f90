...
PURE SUBROUTINE ZHPGVD_F95(AP,BP,W,ITYPE,UPLO,Z,INFO)
    ! Fortran77 call:
    ! ZHPGVD(ITYPE,JOBZ,UPLO,N,AP,BP,W,Z,LDZ,WORK,LWORK,RWORK,LRWORK,
    !   IWORK,LIWORK,INFO)
    ! ITYPE=1,2,3; default: 1
    ! UPLO='U','L'; default: 'U'
    ! <<< Use statements >>>
    USE F77_LAPACK, ONLY: F77_HPGVD, F77_XERBLA
    ! <<< ENTRY point >>>
    ENTRY ZHPGVD_MKL95(AP,BP,W,ITYPE,UPLO,Z,INFO)
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
    ...
    ! <<< Local declarations >>>
    ! <<< Parameters >>>
    CHARACTER(LEN=5), PARAMETER :: SRNAME = 'HPGVD'
    ! <<< Local scalars >>>
    INTEGER :: O_ITYPE
    ...
    LRWORK = -1
    LWORK = -1
    CALL F77_HPGVD(O_ITYPE,JOBZ,O_UPLO,N,AP,BP,W,O_Z,LDZ,S_WORK,LWORK,  &
    ...
    IF(L_STAT_ALLOC==0) THEN
        CALL F77_HPGVD(O_ITYPE,JOBZ,O_UPLO,N,AP,BP,W,O_Z,LDZ,WORK,LWORK,&
