...
PURE SUBROUTINE SGGQRF_F95(A,B,TAUA,TAUB,INFO)
    ...
    ! <<< Use statements >>>
    USE F77_LAPACK, ONLY: F77_GGQRF, F77_XERBLA
    ! <<< ENTRY point >>>
    ENTRY SGGQRF_MKL95(A,B,TAUA,TAUB,INFO)
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
    ...
    LWORK = -1
    CALL F77_GGQRF(N,M,P,A,LDA,O_TAUA,B,LDB,O_TAUB,S_WORK,LWORK,O_INFO)
    ...
    IF(L_STAT_ALLOC==0) THEN
        CALL F77_GGQRF(N,M,P,A,LDA,O_TAUA,B,LDB,O_TAUB,WORK,LWORK,      &
     &                                                           O_INFO)
    ELSE; O_INFO = -1000
    ENDIF
