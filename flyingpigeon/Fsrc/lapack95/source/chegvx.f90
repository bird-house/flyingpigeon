...
PURE SUBROUTINE CHEGVX_F95(A,B,W,ITYPE,UPLO,Z,VL,VU,IL,IU,M,IFAIL,      &
     &                                                      ABSTOL,INFO)
    ! Fortran77 call:
    ! CHEGVX(ITYPE,JOBZ,RANGE,UPLO,N,A,LDA,B,LDB,VL,VU,IL,IU,ABSTOL,M,W,
    ...
    ! <<< Use statements >>>
    USE F77_LAPACK, ONLY: F77_HEGVX, F77_XERBLA
    ! <<< ENTRY point >>>
    ENTRY CHEGVX_MKL95(A,B,W,ITYPE,UPLO,Z,VL,VU,IL,IU,M,IFAIL,ABSTOL,   &
     &                                                             INFO)
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
    ...
    LWORK = -1
    CALL F77_HEGVX(O_ITYPE,JOBZ,RANGE,O_UPLO,N,A,LDA,B,LDB,O_VL,O_VU,   &
     &O_IL,O_IU,O_ABSTOL,O_M,W,O_Z,LDZ,S_WORK,LWORK,RWORK,IWORK,O_IFAIL,&
     &                                                           O_INFO)
    ...
    IF(L_STAT_ALLOC==0) THEN
        CALL F77_HEGVX(O_ITYPE,JOBZ,RANGE,O_UPLO,N,A,LDA,B,LDB,O_VL,    &
     &    O_VU,O_IL,O_IU,O_ABSTOL,O_M,W,O_Z,LDZ,WORK,LWORK,RWORK,IWORK, &
     &                                                   O_IFAIL,O_INFO)
    ELSE; O_INFO = -1000
    ENDIF
