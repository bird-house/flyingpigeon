...
PURE SUBROUTINE DPPSVX_F95(AP,B,X,UPLO,AFP,FACT,EQUED,S,FERR,BERR,RCOND,&
     &                                                             INFO)
    ! Fortran77 call:
    ...
    ! Default S(i)=1.0_WP
    ! <<< Use statements >>>
    USE F77_LAPACK, ONLY: F77_PPSVX, F77_XERBLA
    ! <<< ENTRY point >>>
    ENTRY DPPSVX_MKL95(AP,B,X,UPLO,AFP,FACT,EQUED,S,FERR,BERR,RCOND,    &
     &                                                             INFO)
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
    ...
    IF(L_STAT_ALLOC==0) THEN
     ...
     &                    X,LDX,O_RCOND,O_FERR,O_BERR,WORK,IWORK,O_INFO)
    ELSE; O_INFO = -1000
    ENDIF
