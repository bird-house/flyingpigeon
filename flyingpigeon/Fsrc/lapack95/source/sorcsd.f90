...
PURE SUBROUTINE SORCSD_F95(X11,X12,X21,X22,THETA,U1,U2,V1T,V2T,JOBU1,   &
     &                             JOBU2,JOBV1T,JOBV2T,TRANS,SIGNS,INFO)
    ! Fortran77 call:
    ! SORCSD(JOBU1,JOBU2,JOBV1T,JOBV2T,TRANS,SIGNS,M,P,Q,X11,LDX11,X12,
    !   LDX12,X21,LDX21,X22,LDX22,THETA,U1,LDU1,U2,LDU2,V1T,LDV1T,V2T,
    !   LDV2T,WORK,LWORK,IWORK,INFO)
    ! <<< Use statements >>>
    USE F77_LAPACK, ONLY: F77_ORCSD, F77_XERBLA
    ! <<< ENTRY point >>>
    ENTRY SORCSD_MKL95(X11,X12,X21,X22,THETA,U1,U2,V1T,V2T,JOBU1,JOBU2, &
    ...
    ! <<< Local declarations >>>
    ! <<< Parameters >>>
    CHARACTER(LEN=5), PARAMETER :: SRNAME = 'ORCSD'
    ! <<< Local scalars >>>
    ...
    LWORK = -1
    CALL F77_ORCSD(O_JOBU1,O_JOBU2,O_JOBV1T,O_JOBV2T,O_TRANS,O_SIGNS,M, &
     &P,Q,X11,LDX11,X12,LDX12,X21,LDX21,X22,LDX22,THETA,U1,LDU1,U2,LDU2,&
     &                    V1T,LDV1T,V2T,LDV2T,S_WORK,LWORK,IWORK,O_INFO)
    ...
    ENDIF
    ! Error while build wrapper: SORCSD
    ! <<< Call lapack77 routine >>>
    IF(L_STAT_ALLOC==0) THEN
        CALL F77_ORCSD(O_JOBU1,O_JOBU2,O_JOBV1T,O_JOBV2T,O_TRANS,       &
     &  O_SIGNS,M,P,Q,X11,LDX11,X12,LDX12,X21,LDX21,X22,LDX22,THETA,U1, &
     &         LDU1,U2,LDU2,V1T,LDV1T,V2T,LDV2T,WORK,LWORK,IWORK,O_INFO)
    ELSE; O_INFO = -1000
    ENDIF
